...
public class CreatedUserAdminByLogin implements Estrategy {
    ...
    private static final Log logger = LogFactory.getLog(CreatedUserAdminByLogin.class);
    ...
    public ResponseEntity<ResponseRestAPI> execute(Object object) {
        try {
            ...
            logger.info(presentedUser.toString());
            logger.warn(presentedUser.toString());
            if (userService.findByUsername(presentedUser.getUsername()) == null
                ...
                logger.info("********** SEND REQUEST EMAIL API: " +email.toString()+" *************************");
